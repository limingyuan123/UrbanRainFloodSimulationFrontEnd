...
const changeChooseMap = () => {
  ...
  for (let k = 0; k < rptResult.value.Link.length; k++) {
    ...
    feat.properties.value = linkResultArr.value[k][0];
  }
  // 获取最大最小值
  if (showInPop.NT_showInPop == "Inflow") {
    // node
    nodemin = rptResult.value.MaxMin.node.minInflow;
    nodemax = rptResult.value.MaxMin.node.maxInflow;

    let max = rptResult.value.MaxMin.node.maxInflow;
    nodestep = (nodemax - nodemin) / 5;
  } else if (showInPop.NT_showInPop == "Flooding") {
    nodemin = rptResult.value.MaxMin.node.minFlooding;
    nodemax = rptResult.value.MaxMin.node.maxFlooding;
    nodestep = (nodemax - nodemin) / 5;
  } else if (showInPop.NT_showInPop == "Depth") {
    nodemin = rptResult.value.MaxMin.node.minDepth;
    nodemax = rptResult.value.MaxMin.node.maxDepth;
    nodestep = (nodemax - nodemin) / 5;
  } else if (showInPop.NT_showInPop == "Head") {
    nodemin = rptResult.value.MaxMin.node.minHead;
    nodemax = rptResult.value.MaxMin.node.maxHead;
    nodestep = (nodemax - nodemin) / 5;
  }
  if (showInPop.LT_showInPop == "Flow") {
    // link
    linkmin = rptResult.value.MaxMin.link.minFlow;
    linkmax = rptResult.value.MaxMin.link.maxFlow;
    linkstep = (linkmax - linkmin) / 5;
  } else if (showInPop.LT_showInPop == "Velocity") {
    linkmin = rptResult.value.MaxMin.link.minVelocity;
    linkmax = rptResult.value.MaxMin.link.maxVelocity;
    linkstep = (linkmax - linkmin) / 5;
  } else if (showInPop.LT_showInPop == "Depth") {
    linkmin = rptResult.value.MaxMin.link.minDepth;
    linkmax = rptResult.value.MaxMin.link.maxDepth;
    linkstep = (linkmax - linkmin) / 5;
  } else if (showInPop.LT_showInPop == "Capacity") {
    linkmin = rptResult.value.MaxMin.link.minCapacity;
    linkmax = rptResult.value.MaxMin.link.maxCapacity;
    linkstep = (linkmax - linkmin) / 5;
  }
...
const sliderChange = (val) => {
  ...
  for (let k = 0; k < rptResult.value.Link.length; k++) {
    // 更新geojsonObject
    let feat = geojsonObject.value.features[rptResult.value.Node.length + k];
    feat.properties.value = linkResultArr.value[k][numberAnima.value.value];
  }
  // 更新openlayer
  // refreshOpenlayer();
};
...
const startAnimation = () => {
  btn.startBtn = true;
  btn.pauseBtn = false;
  console.log(nodeResultArr.value);
